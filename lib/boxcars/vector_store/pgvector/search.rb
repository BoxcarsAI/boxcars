    ...
    module Pgvector
      class Search
        ...
        def require_dependencies!
          Boxcars::OptionalDependency.require!("pg", feature: "Boxcars::VectorStore::Pgvector")
          Boxcars::OptionalDependency.require!("pgvector", feature: "Boxcars::VectorStore::Pgvector")
        end
        ...
        def validate_params(params)
          ...
          raise_argument_error('vector_documents must be a hash') unless vector_documents.is_a?(Hash)
          raise_argument_error('type must be pgvector') unless vector_documents[:type] == :pgvector
        ...
        def validate_vector_store(vector_store)
          raise_argument_error('vector_store is nil') unless vector_store
          raise_argument_error('vector_store must be a hash') unless vector_store.is_a?(Hash)
          raise_argument_error('vector_store must have a table_name') unless vector_store[:table_name]
          raise_argument_error('vector_store must have a embedding_column_name') unless vector_store[:embedding_column_name]
          raise_argument_error('vector_store must have a content_column_name') unless vector_store[:content_column_name]
