...
RSpec.describe "Live LLM smoke", :live_env do # rubocop:disable RSpec/DescribeClass
  let(:prompt_text) { "Reply with exactly OK and nothing else." }
      ...
      begin
        ...
        VCR.turned_off do
          output = Timeout.timeout(provider_timeout_seconds) do
            engine_case.fetch(:run).call(prompt_text)
          end
        end
